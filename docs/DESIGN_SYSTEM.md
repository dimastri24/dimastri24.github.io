...
:root {
	...
	--color-cream-200: #f5f7f3; /* Very light forest tint */
	...
	--bg-panel: rgba(21, 41, 30, 0.04);
	--bg-section-light: var(--color-cream-50);
	--bg-section-medium: var(--color-cream-200);
	--bg-section-dark: var(--color-cream-50); /* No full forest backgrounds */
	...
	--radius-md: 0.875rem;
	--radius-lg: 1.25rem;
	/* Removed --radius-pill: 999px; to avoid oversized pill components */

	--border-soft: 1px solid var(--color-line-light);
	--shadow-elevation: 0 2px 8px var(--color-shadow-soft);
...
- **Rhythm:** Subtle section variation only. Use `bg-section-light` for most sections, `bg-section-medium` for subtle variation instead of high-contrast dark/light jumps.
...
- **Reduced Motion:** Mandatory `prefers-reduced-motion` compliance. Remove all non-essential animations. Fallback to immediate removal or short opacity fades.
- **Timing System:** Hover (`140ms`-`180ms`), Reveal (`220ms`-`320ms`), Section-reveal (`320ms`-`420ms`), Loading-exit (`480ms`-`650ms`).
- **Limits:** No WebGL/Three.js, no scroll-jacking, no heavy blur animation, no looping animations or ambient drift.
...
- _Hover (Desktop):_ Subtle color shift to accent over `160ms`. No translations or tint effects to reduce visual noise.
- _Active State:_ Distinct accent line or text color change. Do not rely on hover. Simple transitions on mobile.
...
- **`Button.svelte` / `CvButton.svelte`**: Universal CTA behavior. Hover: subtle background tint change over `160ms`. Active press: returns to baseline instantly.
...
- _Motion:_ Clean reveal animation only. No ambient floating drift.
